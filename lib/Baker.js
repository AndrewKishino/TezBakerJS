 ...
 */
export default class Baker extends Sotez {
    constructor(provider, chain = 'main', net = 'zeronet', options = {}) {
        super(provider, chain);
        /**
         * @description Sets network level constants
         * @param  {String} network The network
         */
        this.getConstants = (network) => {
        ...
        this.revealNonces = (head) => {
            ...
            for (let i = 0; i < this.noncesToReveal.length; i++) {
                ...
                if (head.header.level >= startReveal &&
        ...
        this.levelToCycle = (l) => Math.floor((l - 1) / this.CONSTANTS.cycleLength);
        this.cycleToLevelStart = (c) => c * this.CONSTANTS.cycleLength + 1;
        this.cycleToLevelEnd = (c) => this.cycleToLevelStart(c) + this.CONSTANTS.cycleLength - 1;
        // Run baker
        this.run = () => __awaiter(this, void 0, void 0, function* () {
            ...
            for (let i = 0; i < this.pendingBlocks.length; i++) {
                ...
                if (pendingBlock.level <= this.head.header.level)
                    continue;
                if (this.injectedBlocks.indexOf(pendingBlock.level) >= 0)
                    continue;
                if (this.dateToTime(this.getDateNow()) >=
            ...
            if (this.endorsedBlocks.indexOf(this.head.header.level) < 0) {
                ((head) => __awaiter(this, void 0, void 0, function* () {
                    try {
                        const rights = yield this.query(`/chains/${head.chain_id}/blocks/${head.hash}/helpers/endorsing_rights?level=${head.header.level}&delegate=${this.key.publicKeyHash()}`);
            ...
            if (this.bakedBlocks.indexOf(this.head.header.level + 1) < 0) {
                ((head) => __awaiter(this, void 0, void 0, function* () {
                    try {
                        const rights = yield this.query(`/chains/${head.chain_id}/blocks/${head.hash}/helpers/baking_rights?level=${head.header.level +
                        ...
                        if (this.bakedBlocks.indexOf(head.header.level + 1) < 0) {
                            ...
                            if (this.dateToTime(this.getDateNow()) >=
                                ...
                                this.bake(head, rights[0].priority, rights[0].estimated_time)
                                    ...
                                    this.logOutput(`\x1b[96m-Added potential block for level ${head.header
                                        .level + 1}: [[+${r.data.operations[0].length}], [+${r.data.operations[1].length}], [+${r.data.operations[2].length}], [+${r.data.operations[3].length}]]\x1b[0m`);
                                })
        ...
        this.reveal = (head, nonce) => {
            ...
            return this.query(`/chains/${head.chain_id}/blocks/${head.hash}/helpers/forge/operations`, opOb)
                .then((forgedBytes) => __awaiter(this, void 0, void 0, function* () {
                opOb.protocol = head.protocol;
                const signed = yield this.key.sign(forgedBytes, utility.mergebuf(constants.watermark.endorsement, utility.b58cdecode(head.chain_id, constants.prefix.Net)));
        ...
        this.endorse = (head) => {
            ...
            return this.query(`/chains/${head.chain_id}/blocks/${head.hash}/helpers/forge/operations`, opOb)
                ...
                .then(() => this.query('/injection/operation', sopbytes))
                .then((f) => f)
                .catch((e) => this.logOutput(e));
        };
        this.mempoolCheck = (mempool) => {
            ...
            mempool.applied.forEach((operation) => {
                if (operation.contents.some((op) => op.kind === 'endorsement')) {
                    mempoolEndorsements++;
                }
            });
        ...
        this.bake = (head, priority, timestamp) => __awaiter(this, void 0, void 0, function* () {
            let operations = [[], [], [], []];
            let seed = '';
            let seedHex = '';
            let nonceHash = '';
            ...
            const mempool = yield this.query(`/chains/${head.chain_id}/${this.CONSTANTS.mempool}`);
            ...
            for (let i = 0; i < mempool.applied.length; i++) {
                if (addedOps.indexOf(mempool.applied[i].hash) < 0) {
                    if (mempool.applied[i].branch !== head.hash)
                        continue;
                    if (this.badOps.indexOf(mempool.applied[i].hash) >= 0)
                        continue;
            ...
            let preAppliedBlock;
            try {
                preAppliedBlock = yield this.query(`/chains/${head.chain_id}/blocks/${head.hash}/helpers/preapply/block?sort=true&timestamp=${Math.max(this.dateToTime(this.getDateNow()), this.dateToTime(timestamp))}`, header);
            }
            catch (e) {
                if (e[0] &&
                    e[0].id ===
            ...
            return new Promise(resolve => {
                ...
                // @ts-ignore
                operations = ops;
                return this.query(`/chains/${head.chain_id}/blocks/${head.hash}/helpers/forge_block_header`, shellHeader).then((rr) => __awaiter(this, void 0, void 0, function* () {
                    const forged = rr.block.substring(0, rr.block.length - 22);
                    let signed;
                    let sopbytes;
                    const start = new Date().getTime();
                    this.powLoop(forged, priority, seedHex, (blockbytes, att) => __awaiter(this, void 0, void 0, function* () {
                        const secs = ((new Date().getTime() - start) / 1000).toFixed(3);
                        this.logOutput(`\x1b[96m+POW found in ${att} attempts (${secs} seconds - ${(att /
                            parseFloat(secs) /
                            1000).toFixed(3)} Kh/s)\x1b[0m`);
                        signed = yield this.key.sign(blockbytes, utility.mergebuf(constants.watermark.block, utility.b58cdecode(head.chain_id, constants.prefix.Net)));
                        sopbytes = signed.sbytes;
                        resolve({
                            data: sopbytes,
                            operations,
        ...
        // Utility
        this.powLoop = (forged, priority, seedHex, cb) => {
            ...
            let powLoopHelper;
            (powLoopHelper = (att, syncAtt) => __awaiter(this, void 0, void 0, function* () {
                att++;
                syncAtt++;
                ...
                const checkedHash = yield this.checkHash(hashBuffer);
                if (checkedHash) {
        ...
        this.createProtocolData = (priority, powHeader = '', pow = '', seed = '') => `${priority.toString(16).padStart(4, '0')}${powHeader.padEnd(8, '0')}${pow.padEnd(8, '0')}${seed ? `ff${seed.padEnd(64, '0')}` : '00'}`;
        this.checkHash = (buf) => __awaiter(this, void 0, void 0, function* () {
            try {
                yield _sodium.ready;
            }
            catch (e) {
        ...
        // Utility Functions
        this.dateToTime = (dd) => new Date(dd).getTime() / 1000;
        this.getDateNow = () => `${new Date().toISOString().substr(0, 19)}Z`;
        this.operationPass = (applied) => {
